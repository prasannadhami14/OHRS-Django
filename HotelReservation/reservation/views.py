                ...
                if adult_count is None and child_count is None:
                    ...
                    if error is None:
                        booked=Booking(user=user,
                                    ...
                                    status="pending",
                                     # set the expiration date to 1 minutes from now
                                    expirationDate = datetime.now(pytz.utc) + timedelta(minutes=5)
                        ...
                        if booked.status == "cancled" or booked.status == "CheckOut":
                        ...
                        messagess = f"Hello {name.title()},\n\nWelcome to {rooms.hotel}!\nYour booking for room number {selected_room} of {rooms} type from {date} has been reserved.\nPlease contact hotel  to confirm your reservation as your reservation expires on {booked.expirationDate.strftime('%Y-%m-%d %H:%M')}.\nAdult Count: {adult_count}\nChildren Count: {child_count}\nTotal bill amount : RS.{total_price} \nPayment Type:Pay on property\n\nThank you for visiting us.\nFor any queries contact us:\nMail: infoapps@gmail.com\nPhone: 01-4562398"
                        from_email=settings.EMAIL_HOST_USER
                        to_list=[myuser.email]
                        send_mail(subject,messagess,from_email,to_list,fail_silently=True, auth_user=None, auth_password=None, connection=None, html_message=None)
